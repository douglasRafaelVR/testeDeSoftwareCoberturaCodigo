...
public class Conta implements Cadastro {
    ...
    public void setNumero(String numero) {
        if(!numero.matches("\\d{5}-\\d")){
            throw new IllegalArgumentException("número inválido, deveria estar no seguinte formato 99999-9");
        }
        this.numero = numero;
    }
    ...
    public void setLimite(double limite) { 
        if(!especial && limite > 0){
            throw new IllegalStateException("Somente contas especiais podem fazer isso");
        }
        this.limite = limite;
    }
}